/*
 * RT-Thread T2N1 managed endpoint for the task-2 dual-Guest experiment.
 *
 * The wire format intentionally mirrors components/task2-net-protocol.  This
 * adapter owns only UDP and the endpoint state; the virtio driver and lwIP
 * stack remain RT-Thread responsibilities.
 */
...
static int send_frame(int sockfd, const struct sockaddr_in *peer, uint8_t *frame,
			      size_t length)
{
	return sendto(sockfd, frame, length, 0, (const struct sockaddr *)peer,
			    sizeof(*peer));
}
...
int main(void)
{
	...
	int sockfd;
	int64_t now;
	int64_t last_rx = (int64_t)rt_tick_get_millisecond();
	int64_t last_tx = last_rx;
	int64_t pending_sent = last_rx;

	rt_kprintf("TASK2_MAIN_START\n");
	local.sin_family = AF_INET;
	local.sin_port = htons(UDP_PORT);
	local.sin_addr.s_addr = htonl(INADDR_ANY);
	peer.sin_family = AF_INET;
	peer.sin_port = htons(UDP_PORT);
	if (inet_pton(AF_INET, "10.0.42.15", &peer.sin_addr) != 1) {
		rt_kprintf("TASK2_ERROR invalid peer address\n");
		return 1;
	}
	rt_kprintf("TASK2_SOCKET_CREATE\n");
	sockfd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
	rt_kprintf("TASK2_SOCKET_CREATED fd=%d\n", sockfd);
	if (sockfd < 0 || bind(sockfd, (struct sockaddr *)&local, sizeof(local)) < 0) {
		rt_kprintf("TASK2_ERROR UDP bind errno=%d\n", errno);
		return 1;
	}
	rt_kprintf("TASK2_NET_CONFIGURED interface=virtio-net ip=10.0.42.2/24\n");
	rt_kprintf("TASK2_READY role=managed local=10.0.42.2:4242 peer=10.0.42.15:4242\n");
	if (drop_control_ack_mode == 1) {
		rt_kprintf("TASK2_FAULT_MODE mode=drop-ack-once\n");
	} else if (drop_control_ack_mode == 2) {
		rt_kprintf("TASK2_FAULT_MODE mode=drop-ack-always\n");
	}

	for (;;) {
		struct pollfd pollfd = {.fd = sockfd, .events = POLLIN};
		int poll_result = poll(&pollfd, 1, 100);
		now = (int64_t)rt_tick_get_millisecond();
		if (poll_result > 0 && (pollfd.revents & POLLIN) != 0) {
			struct sockaddr_in source;
			socklen_t source_length = sizeof(source);
			ssize_t received = recvfrom(sockfd, inbound, sizeof(inbound), 0,
			...
			if (received < 0 || parse_frame(inbound, (size_t)received, &kind, &flags,
				...
				rt_kprintf("TASK2_REJECTED malformed_frame\n");
				...
				rt_kprintf("TASK2_SESSION_MISMATCH peer_session=0x%08x ack=%u\n",
				       session_id, sequence);
			} else {
				last_rx = now;
				...
				if (kind == KIND_ACK) {
					...
					if (!valid_heartbeat(payload, payload_length)) {
						...
						(void)send_frame(sockfd, &source, outbound, error_length);
						state_safe = 1;
						rt_kprintf("TASK2_SAFE state=Safe event=InvalidParameter\n");
					} else {
						uint64_t uptime = 0;
						for (unsigned index = 0; index < 8; index++) {
					...
					if (!valid) {
						...
						(void)send_frame(sockfd, &source, outbound, error_length);
						state_safe = 1;
						rt_kprintf("TASK2_PROTOCOL_ERROR invalid_parameter seq=%u\n", sequence);
						...
						if (kind == KIND_CONTROL && drop_control_ack_mode != 0) {
							...
							rt_kprintf("TASK2_FAULT_DROP_ACK%s seq=%u\n",
							       TASK2_FAULT_DROP_ACK_MODE == 2 ? "_ALWAYS" : "",
							       sequence);
						} else {
							(void)send_frame(sockfd, &source, outbound, ack_length);
						}
						...
						pending_sent = now;
						retry_count = 0;
						(void)send_frame(sockfd, &source, pending_frame, pending_length);
						last_tx = now;
						rt_kprintf("TASK2_CONTROL_RECEIVED seq=%u request=%u\n", sequence,
						       request);
						...
						if (kind == KIND_CONTROL && drop_control_ack_mode == 2) {
							rt_kprintf("TASK2_FAULT_DROP_ACK_ALWAYS seq=%u duplicate=true\n",
							       sequence);
						} else {
							(void)send_frame(sockfd, &source, outbound, ack_length);
						}
						...
						(void)send_frame(sockfd, &source, outbound, error_length);
						state_safe = 1;
						rt_kprintf("TASK2_PROTOCOL_ERROR out_of_order=%u expected=%u\n", sequence,
						       expected_rx_sequence);
					}
				} else {
					rt_kprintf("TASK2_REJECTED unsupported_kind=%u\n", kind);
				}
			}
		}
		...
		if (pending_length > 0 && now - pending_sent >= RETRANSMIT_AFTER_MS) {
			if (retry_count >= MAX_RETRIES) {
				pending_length = 0;
				state_safe = 1;
				rt_kprintf("TASK2_SAFE state=Safe event=RetryExhausted seq=%u\n", pending_sequence);
			} else {
				retry_count++;
				pending_sent = now;
		...
		if (!state_safe && now - last_rx >= PEER_TIMEOUT_MS) {
			state_safe = 1;
			rt_kprintf("TASK2_SAFE state=Safe event=HeartbeatTimeout\n");
		}
		if (now - last_tx >= HEARTBEAT_INTERVAL_MS) {
			size_t heartbeat_length = make_heartbeat(outbound, (uint64_t)now);
			(void)send_frame(sockfd, &peer, outbound, heartbeat_length);
			last_tx = now;
		}
	}
}
